...
authorizationSchema.methods.NewCred = function(login) {
    var self = this;
    if (!self.AccessKeyId || !self.SecretKey || !self.SessionToken) {
        var params = {
            IdentityPoolId: login.IdentityPoolId,
            IdentityId: self._id,
            Logins: ci.NewLogins("Origin", self._token),
        };
        return ci.GetCredentials(params).then(function(cred) {
            self.AccessKeyId = cred.data.Credentials.AccessKeyId;
            self.SecretKey = cred.data.Credentials.SecretKey;
            self.SessionToken = cred.data.Credentials.SessionToken;
            return self.save();
        })
        .then(function() {
            return new aws.Credentials(self.AccessKeyId, self.SecretKey, self.SessionToken);
        });
    } else {
        return Promise.resolve(
            new aws.Credentials(self.AccessKeyId, self.SecretKey, self.SessionToken)
        );
    }
};
