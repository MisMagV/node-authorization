    ...
    if (self._credentials && self._credentials.valid()) {
        ...
        return ci.GetCredentials(params)
            .then(function(c) {
                ...
                self._credentials = new Credentials({
                    // inherit this identity from predecessor
                    _id: self._credentials._id.valueOf(),
                    // Credential data for AWS asset
                    AccessKeyId: c.data.Credentials.AccessKeyId,
                    SecretKey: c.data.Credentials.SecretKey,
                    SessionToken: c.data.Credentials.SessionToken,
                    // Extra info for credential expire datetime
                    Expiration: c.data.Credentials.Expiration,
                });
                return self._credentials.save({duration: credDuration});
            })
            .then(function() {
                return self._credentials.get();
            });
    ...
    Auth.OpenID = function(login, accnt) {
        return ci.GetOpenIdTokenForDeveloperIdentity(login).then(function(id) {
            var authorize = new Auth({
                ...
                _credentials: new Auth.Credentials({}),
                // User's current rights
                rights: accnt.rights,
            });
            // TODO: purge old Credentials?
            return authorize;
        });
    };
